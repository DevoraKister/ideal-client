    ...
    public class Login
    {
     ...
     //   }
        ...
        public static bool NotValidPassword(string pw)
        {
            ...
            List<Entities.User> user = new List<Entities.User>();
            var x = boss.FirstOrDefault(e => e.password.Equals(pw));
            if (x != null)
                return false;
            var y = user.FirstOrDefault(e => e.UserPassword.Equals(pw));
            if (y != null)
                return false;
            return true;
        ...
        {
            List<Entities.Boss> boss = new List<Entities.Boss>();
            var x = boss.Find(p => p.password.Equals(pw) && p.BossName.Equals(name));
            if (x != null)
                return true;
            return false;
